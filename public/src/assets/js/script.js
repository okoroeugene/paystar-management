...
(function ($) {
    ...
    var $currentThemeStyle = ($dtStyle) ? $dtStyle : 'style-7x';
...
function changeTheme($theme, $style, $themeStylesheet, $init) {
    ...
    if ($theme === 'lite') {
        ...
        $themeStylesheet.href = 'src/assets/css/' + $theme + '-' + $style + '.min.css';
    } else if ($theme === 'dark') {
        $logo.attr('src', 'https://via.placeholder.com/90x18');
        $body.removeClass('theme-semidark').addClass('theme-dark');
        $themeStylesheet.href = 'src/assets/css/' + $theme + '-style-7x.min.css';
